...
import type { RankedContract } from './types';
...
export function norm(x: number, lo: number, hi: number) {
  if (hi <= lo) return 0.5;
  return Math.max(0, Math.min(1, (x - lo) / (hi - lo)));
}

export function dte(expiration: string) {
  const exp = new Date(expiration.slice(0, 10));
  const now = new Date();
  return Math.ceil((exp.getTime() - now.getTime()) / 86_400_000);
}

/** OI proxy when IBKR/synth chains report zero open interest. */
export function effectiveOi(row: OptionRow, spot: number): number {
  if (row.openInterest > 0) return row.openInterest;
  const strike = row.strike || spot;
  const m = spot ? strike / spot - 1 : 0;
  const center = row.type === 'PUT' ? -0.012 : 0.012;
  const weight = row.type === 'PUT' ? 1.35 : 1;
  return 1000 * weight * Math.exp(-(((m - center) * 8) ** 2));
}
...
export function mcContract(spot: number, row: OptionRow, premium: number, dteDays: number, seed: number) {
...
export function parseExpiration(id: string): string {
  return id.match(/(\d{4}-\d{2}-\d{2})/)?.[1] ?? '';
}

export function historicalWinRateFromCloses(closes: number[]): { winRate: number; bullish: boolean } {
...
export function historicalEdgeFactor(
  row: OptionRow,
...
export function scoreRow(
  ...
  const oi = effectiveOi(row, spot);
  if (oi < 10 || row.mid < 0.05) return null;
  ...
  const factors = {
    ...
    liquidity: norm(Math.log1p(row.volume) + Math.log1p(oi), 0, 12),
    spread_quality: norm(1 - spreadPct, 0.82, 1),
    gamma_positioning: norm(Math.abs(row.gamma) * oi, 0, 500),
